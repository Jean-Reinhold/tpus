...
**Deck:** `deliverable-assets/presentations/2026-07-15-arquitetura-de-tpus/out/deliverable.pdf` (21 slides)
**Duração alvo:** 16 minutos (dá para fazer em 14 acelerando as transições; 19 com perguntas)

## O que assistir e ler antes (em ordem)

1. **[CPU vs GPU vs TPU](https://www.youtube.com/watch?v=MUWAbpg1xLo)** (vídeo curto). Dá a intuição da diferença entre os três chips. É a base dos slides 6 e 8.
2. **[An in-depth look at Google's first TPU](https://cloud.google.com/blog/products/ai-machine-learning/an-in-depth-look-at-googles-first-tensor-processing-unit-tpu)** (artigo, Google Cloud Blog). A fonte principal do deck: diagrama de blocos, array sistólico, INT8, CISC e os números de desempenho. Slides 10, 12, 13 e 18 saem daqui.
3. **[All about AI Accelerators, com Adi Fuchs](https://www.youtube.com/watch?v=VQoyypYTz2U)** (entrevista longa). Contexto geral de aceleradores. Não precisa ver inteiro: foque nos trechos sobre TPU e dataflow, que ajudam a defender o slide 15 (Flynn).
4. **[How to Think About TPUs](https://jax-ml.github.io/scaling-book/tpus/)** (capítulo do JAX Scaling Book). A melhor referência para a TPU moderna: TensorCore, VMEM, HBM, ICI e a topologia em toro. Slides 16 e 17.
...
7. **Atenção:** os slides 4, 5 e 19 (Almir Salek, a parceria com a Broadcom e os serviços da Google) não vêm da bibliografia acima — confirme essas informações em fonte própria antes de apresentar como fato.
...
| 1:00 a 5:40 | 3 a 8 | Fundamentos: por que o Google fez seu próprio chip, quem construiu, a parceria com a Broadcom, o que é uma TPU, redes neurais, CPU × GPU × TPU |
| 5:40 a 10:35 | 9 a 13 | Por dentro do chip: blocos, matmul e ReLU, array sistólico, eficiência |
| 10:35 a 14:30 | 14 a 17 | Flynn, TPU moderna e conexão em toro |
| 14:30 a 17:00 | 18 a 21 | Números, serviços que usam TPU, síntese e encerramento |

Regra de bolso: uns 45 a 75 segundos por slide de conteúdo. Os divisores (3, 9, 14) são respiros de 10 segundos: anuncie a seção e siga.
...
### 4 · Quem ajudou a construir a TPU (30 s)
- Antes de definir o que é o chip, uma referência rápida a quem esteve por trás dele: Almir Salek.
- Ele fundou e escalou a organização de System-on-a-Chip (SoC) da NVIDIA, e depois entregou as primeiras 7 gerações de soluções TPU para os data centers do Google.
- Não se demore aqui — é contexto, não o foco da apresentação.

### 5 · A parceria por trás da primeira TPU (30 s)
- Em 2014 a Google fechou parceria com a Broadcom para a fabricação da primeira geração de TPUs.
- Frase de transição: "o Google desenhou o chip, mas não fabricou sozinho — como boa parte da indústria de semicondutores, contou com um parceiro de manufatura."

### 6 · O que é uma TPU (60 s)
...
### 7 · O que uma rede neural calcula (75 s)
...
- Destaque a latência determinística (sem cache nem especulação) e a linha Flynn: "guardem essa, ela volta no slide 15".
...
### 9 · Divisor: Por dentro do chip (10 s)
> "Agora vamos abrir o chip."

### 10 · Diagrama de blocos da TPU v1 (75 s)
...
### 11 · A conta que o array sistólico resolve (60 s)
- Mostre o exemplo numérico: c₂₁ é a linha 2 de A vezes a coluna 1 de B, somadas — um produto escalar por célula de C. É exatamente essa soma que uma célula MAC calcula a cada ciclo.
- Ao lado, a curva da ReLU: a não linearidade aplicada célula a célula, depois de cada Y = W·X + b.
- Feche com a ponte: cada saída do array sistólico (próximo slide) é uma dessas somas MAC, pronta para passar pela ReLU antes da próxima camada.

### 12 · Array sistólico (90 s), o slide mais importante
...
### 13 · Três escolhas que explicam a eficiência (60 s)
...
### 19 · Serviços da Google que utilizam TPU (45 s)
- Google Cloud TPU: disponibiliza as TPUs como recurso de computação escalável para empresas treinarem e executarem modelos massivos de IA na nuvem.
- Kaggle: dá aos pesquisadores acesso gratuito a TPUs para treinar redes neurais dentro da própria plataforma.
- Google Colab: notebooks Python na nuvem com aceleração por hardware TPU.
> "Não é só pesquisa interna — a TPU virou produto, disponível em vários níveis de acesso."

### 20 · Síntese e fontes (45 s)
...
5. **"Por que o Google terceirizou a fabricação da primeira TPU?"** Projetar um ASIC não exige uma fábrica própria: a Broadcom já tinha a expertise de fabricação e design de ASIC sob encomenda, o que deixou o Google livre para focar na arquitetura enquanto a manufatura ficava com um parceiro especializado.
